                  ...
                  {subject.topics.sort((a: Topic, b: Topic) => (a.orderIndex ?? 999) - (b.orderIndex ?? 999)).map((topic: Topic) => {
                    ...
                    return (
                      <button key={topic.id} onClick={() => selectTopic(topic.slug)}
                        className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-left transition-all', active ? 'bg-blue-600 text-white font-medium' : 'text-foreground hover:bg-accent')}>
                        {done && !active ? <CheckCircle2 className="w-3 h-3 flex-shrink-0 text-green-500" /> : <Circle className={cn('w-3 h-3 flex-shrink-0', active ? 'text-blue-200' : 'text-muted-foreground')} />}
...
function CpContent() {
  ...
  const selectTopic = (slug: string) => { const p = new URLSearchParams(searchParams.toString()); p.set('topic', slug); router.push(`/cp?${p.toString()}`, { scroll: false }); setMobileOpen(false); };
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold">Competitive Programming</h1><p className="text-muted-foreground text-sm mt-1 hidden sm:block">Master advanced algorithms and techniques for coding contests.</p></div>
        ...
        <main className="flex-1 min-w-0 border border-border rounded-xl p-6 bg-card">
          {!selectedSlug ? (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
              ...
              <button onClick={() => setMobileOpen(true)} className="md:hidden flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium"><Menu className="w-4 h-4" /> Browse topics</button>
            </div>
          ) : topicLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => bookmarkMutation.mutate()} disabled={bookmarkMutation.isPending} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors', isBookmarked ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' : 'border border-border hover:bg-accent text-muted-foreground hover:text-foreground')}>{isBookmarked ? <><BookmarkCheck className="w-3.5 h-3.5" /> Bookmarked</> : <><Bookmark className="w-3.5 h-3.5" /> Bookmark</>}</button>
                  </div>
                </div>
                ...
                {topicData.topic.shortDescription && <p className="text-muted-foreground text-sm border-l-2 border-blue-400 pl-3">{topicData.topic.shortDescription}</p>}
              </div>
