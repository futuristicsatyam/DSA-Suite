...
function SidebarContent({ subjects, subjectsLoading, selectedSlug, selectTopic, expanded, toggleExpand, completedTopicIds, isAuthenticated }: any) {
  return (
...
function LanguageContent({ langSlug }: { langSlug: string }) {
  ...
  const { data: course, isLoading: courseLoading, error: courseError } = useQuery({
    queryKey: ['language', langSlug],
    queryFn: () => api.get(`/content/courses/${langSlug}`).then(r => r.data as LanguageCourse),
    staleTime: 5 * 60_000,
  });

  const { data: subjects = [], isLoading: subjectsLoading } = useQuery({
    queryKey: ['language-subjects', langSlug],
    queryFn: () => api.get(`/content/courses/${langSlug}/subjects`).then(r => r.data as Subject[]),
    staleTime: 5 * 60_000,
    enabled: !!course,
  });

  const { data: topicData, isLoading: topicLoading } = useQuery({
    queryKey: ['language-topic', langSlug, selectedSlug],
    queryFn: () => api.get(`/content/courses/${langSlug}/topics/${selectedSlug}`).then(r => r.data as { topic: Topic & { id: string }; editorial: Editorial | null }),
  ...
  const { data: practiceProblems = [] } = useQuery({
    queryKey: ['language-problems', topicData?.topic?.id],
  ...
  const enrollMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(enrollmentStatus?.enrolled ? 'Unenrolled' : 'Enrolled successfully! 🎉');
    },
  ...
  const selectTopic = (slug: string) => {
    ...
    p.set('topic', slug);
    router.push(`/languages/${langSlug}?${p.toString()}`, { scroll: false });
    setMobileOpen(false);
  };
  ...
  if (courseError) {
    return (
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-12 text-center">
        ...
        <p className="text-muted-foreground mb-6">The language you're looking for doesn't exist or has been removed.</p>
        <Link href="/" className="text-indigo-600 hover:underline">Go back home</Link>
      </div>
    );
  }

  const sidebarProps = { subjects, subjectsLoading, selectedSlug, selectTopic, expanded, toggleExpand, completedTopicIds, isAuthenticated };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1 hidden sm:block">
            {course?.description || 'Explore subjects and topics in this language.'}
          </p>
        </div>
        ...
        <main className="flex-1 min-w-0">
          {!selectedSlug ? (
            <div className="border border-border rounded-xl bg-card overflow-hidden">
              {course?.thumbnail ? (
                <div className="w-full aspect-video"><img src={course.thumbnail} alt={course?.name || 'Language'} referrerPolicy="no-referrer" className="w-full h-full object-cover" /></div>
              ) : (
                <div className="w-full aspect-video bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center"><Code2 className="w-20 h-20 text-white/50" /></div>
              )}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-6 text-sm text-muted-foreground mt-3">
                      ...
                      <span className="flex items-center gap-1.5"><Code2 className="w-4 h-4" /> {subjects.reduce((s: number, sub: Subject) => s + sub.topics.length, 0)} topics</span>
                    </div>
                  </div>
                  <div className="flex-shrink-0">
                    {isAuthenticated ? (
                      enrollmentStatus?.enrolled ? (
                        ...
                        <a href={course.ctaUrl} target="_blank" rel="noopener noreferrer" className="inline-block px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium text-center transition-colors">{course.ctaText || 'Enroll Now'}</a>
                      ) : (
                        <button onClick={() => enrollMutation.mutate()} disabled={enrollMutation.isPending} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50">{enrollMutation.isPending ? 'Enrolling...' : (course?.ctaText || 'Enroll Now')}</button>
                      )
                    ) : (
...
export default function LanguagePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <Suspense fallback={
      ...
      <LanguageContent langSlug={resolvedParams.slug} />
    </Suspense>
  );
}
