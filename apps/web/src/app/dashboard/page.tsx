  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link href="/profile" className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center text-lg font-bold hover:bg-indigo-700 transition-colors">
      ...
      {data && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-3">
          ...
          <div className="h-2 bg-secondary rounded-full overflow-hidden">
            <div className="h-full bg-indigo-600 rounded-full transition-all duration-500" style={{ width: `${data.progressSummary.percent}%` }} />
          </div>
        </div>
      )}
            ...
            {data.weeklyActivity.map((day) => {
              ...
              return (
                <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                  <div className={cn('w-full rounded-sm transition-all', day.count > 0 ? 'bg-indigo-600' : 'bg-secondary')} style={{ height: `${height}px` }} />
                  <span className="text-xs text-muted-foreground">{new Date(day.date).toLocaleDateString('en', { weekday: 'short' })[0]}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          <h2 className="font-semibold text-sm flex items-center gap-2"><Clock className="w-4 h-4 text-muted-foreground" /> Continue learning</h2>
          {isLoading ? <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-12 bg-muted animate-pulse rounded-lg" />)}</div>
            : data?.continueLearning.length === 0 ? (
              ...
              <div className="space-y-2">
                {data?.continueLearning.slice(0, 4).map((item: any) => (
                  <Link key={item.id} href={`/${item.topic.subject.categoryType.toLowerCase()}?topic=${item.topic.slug}`}
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-sm flex items-center gap-2"><Bookmark className="w-4 h-4 text-muted-foreground" /> Bookmarks</h2>
            ...
            : data?.bookmarks.length === 0 ? (
              ...
              <div className="space-y-2">
                {data?.bookmarks.slice(0, 4).map((item: any) => (
                  <Link key={item.id} href={`/${item.topic?.subject?.categoryType?.toLowerCase() ?? 'dsa'}?topic=${item.topic?.slug}`}
                    className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent transition-colors group">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{item.topic?.title}</p>
                      <p className="text-xs text-muted-foreground">{item.topic?.subject?.name}</p>
...
export default function DashboardPage() {
  return (
    <Suspense fallback={<div className="max-w-5xl mx-auto px-4 py-10 animate-pulse space-y-4"><div className="h-8 bg-muted rounded w-48" /></div>}>
      <DashboardContent />
    </Suspense>
  );
}
